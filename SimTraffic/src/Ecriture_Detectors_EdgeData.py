...
def clearAllRectangles(id_lane):
    global allRectangles
    allRectangles = [rectangle for rectangle in allRectangles if rectangle[0] != id_lane]

def idLane(detectorCoordinates): #CoordSumoToidLane
...
url = "https://opendata.paris.fr/api/records/1.0/search/?dataset=comptages-routiers-permanents&q=&sort=t_1h&facet=q&geofilter.distance=++++{}%2C+++++{}%2C+{}".format(parameters[1], parameters[2], parameters[3])
...
with open('edgedata.csv', 'w', newline='') as csvfile: #Pour le fichier edgedata.csv
    ...
    for key in donnees.records:    #On regarde tous les détecteurs
        ...
        coordS = coordSumo(key['fields']['geo_point_2d'][0], key['fields']['geo_point_2d'][1])
        idL = idLane(coordS)
    ...
    while len(allRectangles)>0:
            ...
            detectorsDefinition.set('pos', '0')
            #Ligne dans edgedata.csv_______________________________________
            data.writerow([str(i), '0', moy])
            i+=1
            clearAllRectangles(rectangle[0]) #Il y avait plusieurs fois la même lane dans allRectangles
